...
Limno_SCUO <- codonO_2_df(pathx = "./IMG_annotation/IMG_2724679690_Limnohabitans/Annotation/",
                          patternx = "codonO")

# Merge dataframes
ref_limno_SCUO <- rbind(ref_SCUO, Limno_SCUO)
...
ref_limno_SCUO$Genome[ref_limno_SCUO$Genome %in% "2724679690.genes.fna.codonO.output"] <- "Lim. MAG (2724679690)"
...
<img src="Figures/cached/compare-limno-CB-1.png" style="display: block; margin: auto;" />

```r
# Plot SCUO-GC profiles
# p_limno_SCUO_GC <- ref_limno_SCUO  %>% 
#   ggplot(aes(x = GC, y = SCUO, fill = Genome))+
#   theme_bw()+
#   geom_point(size = 4, shape = 21, color = "black", alpha = 0.4)+
#   scale_fill_brewer(palette = "Paired")+
#   theme(legend.position="bottom", axis.text.x = element_text(angle = 60, hjust = 1))+
#   guides(fill=FALSE)+
#   ggtitle("Codon bias - SCUO")+
#   facet_grid(GCx~Genome)+
...
ord_list_bin <- c("Lim sp. Rim11","Lim. sp. 103DPR2",
                  "Lim. MAG", "Lim. sp. DM1",
                  "Lim. planktonicus II-D5", "Lim. sp. 2KL-3", 
                  "Lim. sp. Rim28",  "Lim sp. Rim47",
                  "Lim. sp. 2KL-27", "Lim. sp. 63ED37-2")
...
p_ani <- ggplot(data = df_pyani,
  ...
  theme(axis.title=element_text(size=16), strip.text.x=element_text(size=16),
        ...
        axis.text.x = element_text(size=20, angle = 55, hjust = 0),
