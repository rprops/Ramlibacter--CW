...
```{r Codon bias, dpi = 300, warning = FALSE, fig.width = 12, fig.height = 13}
...
SCUO_merged <- dplyr::left_join(SCUO_merged, merged_gc_ko[, c(1:2,4 ,14:21)], by = c("Gene" = "contig_geneID"))
...
# p_SCUO.2 <- ggplot(data = SCUO_merged_sb, aes (x = 100*GC.x, y = SCUO, fill = level_B))+
#   geom_point(size = 4, shape = 21, alpha = 0.7)+
#   scale_fill_brewer(palette = "Accent")+
#   # scale_fill_manual(values = c(col_bac1, col_bac2, col_limno))+
#   theme_bw()+
